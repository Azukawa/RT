...
static void	calculate_lighting(t_rt *rt, t_ray *ray, t_color *c)
{
	...
	lr.start = v_add(ray->start, v_mult(n, SHAD_BIAS));
	lr.dir = v_normalize(dist);
	if (in_shadow(rt, lr, dist))
		return ;
	*c = col_blend(*c, assign_color(rt, lr, n, *c), 1);
}
...
static t_color	ray_col(t_rt *rt, float t)
{
	...
	else
	{
		...
		mixer = col_add(mixer,
				col_mult_colors(rt->object[rt->curobj].color, rt->amb_col), 1);
		if (rt->object[rt->curobj].tx_type == TEX)
			mixer = col_blend(mixer, apply_sqr_tex(rt, rt->t_scale), 0.5);
		else if (rt->object[rt->curobj].tx_type == CHECK)
			mixer = col_blend(mixer, apply_check(rt, rt->t_scale, mixer), 0.5f);
	}
	if (rt->mir_hit == TRUE)
		mixer = col_blend(mixer, rt->mir_image, 0.15);
	return (mixer);
...
void	raytracer(t_rt *rt, int x, int y)
{
	...
	draw_pixel(x, y, &rt->rend.win_buffer, \
		convert_to(col_to_uint(ray_col(rt, t)), &rt->col_switch));
}
